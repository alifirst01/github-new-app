...
export default class Issues extends Vue {
  ...
  async getGithubAccessToken(accessCode: string): Promise<any> {
    this.loading = 0;
    this.loadingMsg = {
      m1: "Finishing GitHub login",
      m2: "It should only be a second or two…"
    };
    this.$Progress.start();
    return await this.githubAuthController
      .getGithubAccessToken(accessCode)
  ...
  async getAllUserIssues() {
    ...
    return await this.issuesController
      .getUserIssues()
      .then((issuesResult: GetIssuesResult) => {
